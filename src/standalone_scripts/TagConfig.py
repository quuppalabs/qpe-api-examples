...
tag_data_url = urlparse.urljoin(qpe_server, 'qpe/getTagData?mode=json&format=defaultInfo')
config_url = urlparse.urljoin(qpe_server, 'qpe/configureTag')
group_url = urlparse.urljoin(qpe_server, '/qpe/setTagGroup')

################# Query Parameters #################
config_id = 'ASSET_TAG' #the name of the configuration in the project to configure with
...
def main():
    ...
    while True:
        ...
        for tag in tag_data:
            ...
            if tag['tagGroupName'] is None and tag['tagId'] not in config_proccess_tags and tag['tagId'] not in ungrouped_tags:
